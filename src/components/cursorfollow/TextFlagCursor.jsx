...
const TextFlagCursor = (options) => {
  ...
  useEffect(() => {
    ...
    // Add resize event listener to update the state
  ...
  useEffect(() => {
    // If it's mobile, no need to run the cursor effect
    if (isMobile) {
      return;
    }
    ...
    let text = cursorOptions.text ? " " + options.text : " Welcome to Marketiq";
    let color = options?.color || "#013220";
    let font = cursorOptions.font || "monospace";
    let textSize = cursorOptions.textSize || 14;
  ...
  }, [options, isMobile]); // add isMobile to dependency array
